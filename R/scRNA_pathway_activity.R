...
pathway_file <- "/Users/zhiruiguo/UW-Madison/23fall/DinhLab/metabolic_lab_data/KEGG_metabolism.gmt"
...
norm_rds_file <- file.path("/Users/zhiruiguo/UW-Madison/23fall/DinhLab/metabolic_lab_data/",paste0(normalization_method,"_tpm.rds"))
...
for(p in pathway_names){
  ...
  if (is.vector(pathway_metabolic_tpm)) next
...
dat[is.na(dat)] <- 1
pdf(file.path("/Users/zhiruiguo/UW-Madison/23fall/DinhLab/metabolic_lab_data/", "KEGGpathway_activity_heatmap.pdf"),onefile=T,width=6,height=9)
mybreaks <- c(
...
pheatmap(dat[sort_row,sort_column],cluster_cols = TRUE,cluster_rows = F,color=color,breaks=mybreaks)
dev.off()

write.table(mean_expression_noshuffle,file=file.path("/Users/zhiruiguo/UW-Madison/23fall/DinhLab/metabolic_lab_data/","KEGGpathway_activity_noshuffle.txt"),row.names=T,col.names=T,quote=F,sep="\t")
write.table(mean_expression_shuffle,file=file.path("/Users/zhiruiguo/UW-Madison/23fall/DinhLab/metabolic_lab_data/","KEGGpathway_activity_shuffle.txt"),row.names=T,col.names=T,quote=F,sep="\t")
write.table(pvalues_mat,file=file.path("/Users/zhiruiguo/UW-Madison/23fall/DinhLab/metabolic_lab_data/","KEGGpathway_activity_shuffle_pvalue.txt"),row.names=T,col.names=T,quote=F,sep="\t")
...
ggsave(file.path("/Users/zhiruiguo/UW-Madison/23fall/DinhLab/metabolic_lab_data/","pathway_activity_violinplot.pdf"),g,width = 2.5,height=1.5,units="in",device="pdf",useDingbats=FALSE)
